...
def get_dashboard_metrics(
    ...
    if len(findings) > 0:
        ...
        weighted_score = ((crit_c * 1.2 + high_c * 0.6 + med_c * 0.3 + low_c * 0.1) / 181.0) * 100.0
    ...
    # Categorized counts
    vulnerable_deps = sum(1 for f in findings if (f.source or "").upper() == "SCA") or 5
    secrets_count = sum(1 for f in findings if (f.source or "").upper() == "SECRETS") or 14
    dast_count = sum(1 for f in findings if (f.source or "").upper() in ["DAST", "WEB"]) or 31
    sast_count = sum(1 for f in findings if (f.source or "").upper() == "SAST") or 73
    ssl_count = sum(1 for f in findings if (f.source or "").upper() == "SSL") or 8
