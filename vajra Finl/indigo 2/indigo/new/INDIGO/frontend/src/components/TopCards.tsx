    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      {cards.map((card, index) => (
        <div
          ...
          className="bg-card border-[2.5px] border-[#360a25] rounded-xl p-4.5 hover:border-[#ff1744] transition-all duration-300 hover:shadow-[0_0_22px_rgba(255,23,68,0.4)] cursor-pointer flex flex-col justify-between group relative overflow-hidden"
        >
          <div className="flex items-start justify-between mb-2">
            <div className="flex-1 min-w-0">
              <p className="text-[11px] text-zinc-400 uppercase tracking-widest font-black truncate mb-1">
                {card.label}
              </p>
              <p className="text-2xl font-black text-white tracking-tight group-hover:text-[#ff1744] text-glow transition-colors">
                {card.value}
              </p>
            </div>
            <div className={`p-2.5 rounded-xl ${card.bgColor} border border-[#360a25] flex-shrink-0 ml-3 shadow-inner`}>
              <card.icon className={`w-5 h-5 ${card.color}`} />
            </div>
          </div>

          <div className="flex items-center justify-between pt-2.5 border-t border-[#360a25] text-xs">
            <span className="text-zinc-400 font-bold text-[11px] truncate tracking-wide">{card.subtext}</span>
            {card.trend && (
              <span className={`font-black text-[11px] ${card.color} ml-2 flex-shrink-0 font-mono tracking-wider`}>
                {card.trend}
              </span>
            )}
          </div>
