...
function renderMarkdown(text: string): React.ReactNode {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('# ')) { elements.push(<h1 key={i} className="text-sm font-bold text-white mb-1 mt-1 border-b border-[#360a25] pb-1">{inlineFormat(line.slice(2))}</h1>); i++; continue }
    if (line.startsWith('## ')) { elements.push(<h2 key={i} className="text-xs font-bold text-[#ff1744] mb-1 mt-2">{inlineFormat(line.slice(3))}</h2>); i++; continue }
    if (line.startsWith('### ')) { elements.push(<h3 key={i} className="text-xs font-semibold text-[#00f2fe] mb-0.5 mt-1.5">{inlineFormat(line.slice(4))}</h3>); i++; continue }
    if (line.startsWith('```')) {
      const codeLines: string[] = []; i++
      while (i < lines.length && !lines[i].startsWith('```')) { codeLines.push(lines[i]); i++ }
      elements.push(<pre key={`code-${i}`} className="bg-[#030004] border border-[#360a25] rounded-lg p-2.5 my-1.5 overflow-x-auto"><code className="text-[10px] font-mono text-[#00ff88] leading-relaxed">{codeLines.join('\n')}</code></pre>)
      i++; continue
    }
    if (/^[-*•] /.test(line)) {
      const bullets: string[] = []
      while (i < lines.length && /^[-*•] /.test(lines[i])) { bullets.push(lines[i].replace(/^[-*•] /, '')); i++ }
      elements.push(<ul key={`ul-${i}`} className="space-y-0.5 my-1 pl-1">{bullets.map((b, bi) => (<li key={bi} className="flex items-start gap-1.5 text-[11.5px] text-zinc-300 leading-relaxed"><span className="w-1.5 h-1.5 rounded-full bg-[#ff1744] mt-1.5 flex-shrink-0" /><span>{inlineFormat(b)}</span></li>))}</ul>)
      continue
    }
    if (/^\d+\. /.test(line)) {
      const items: string[] = []
      while (i < lines.length && /^\d+\. /.test(lines[i])) { items.push(lines[i].replace(/^\d+\. /, '')); i++ }
      elements.push(<ol key={`ol-${i}`} className="space-y-0.5 my-1 pl-1">{items.map((item, idx) => (<li key={idx} className="flex items-start gap-2 text-[11.5px] text-zinc-300 leading-relaxed"><span className="text-[9px] font-bold text-white bg-[#ff1744]/20 border border-[#ff1744]/40 rounded w-4 h-4 flex items-center justify-center flex-shrink-0 mt-0.5">{idx + 1}</span><span>{inlineFormat(item)}</span></li>))}</ol>)
      continue
    }
    if (line.startsWith('> ')) { elements.push(<blockquote key={i} className="border-l-2 border-[#ff1744] pl-2.5 my-1 text-[11px] text-zinc-400 italic bg-[#060108]/60 py-0.5">{inlineFormat(line.slice(2))}</blockquote>); i++; continue }
    if (/^---+$/.test(line.trim())) { elements.push(<hr key={i} className="border-[#360a25] my-2" />); i++; continue }
    elements.push(<p key={i} className="text-[11.5px] text-zinc-300 leading-relaxed">{inlineFormat(line)}</p>)
    i++
  }
...
function inlineFormat(text: string): React.ReactNode {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    else if (token.startsWith('`')) parts.push(<code key={match.index} className="bg-[#030004] border border-[#360a25] px-1.5 py-0.5 rounded text-[10.5px] font-mono text-[#00f2fe]">{token.slice(1, -1)}</code>)
    else if (token.startsWith('*')) parts.push(<em key={match.index} className="italic text-zinc-400">{token.slice(1, -1)}</em>)
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isSpeaking && isOpen && (
          <motion.button
            ...
            className="fixed bottom-6 right-24 z-50 flex items-center gap-2 bg-[#ff1744] hover:bg-[#ff1744]/90 text-white text-xs font-black px-4 py-3 rounded-full shadow-[0_0_20px_rgba(255,23,68,0.6)] border border-white/20 transition-all uppercase tracking-wider"
      ...
      <motion.button
        ...
        className={`fixed bottom-6 right-6 w-14 h-14 bg-gradient-to-br from-[#ff1744] via-[#be123c] to-[#880815] rounded-full flex items-center justify-center shadow-[0_0_24px_rgba(255,23,68,0.55)] z-50 transition-all border border-white/25 ${fabRing}`}
      ...
      >
        {isOpen ? (
          ...
          <div className="relative flex items-center justify-center">
            ...
            <span className="absolute -top-1.5 -right-2 text-[8px] font-black font-mono px-1 py-0.2 bg-[#00f2fe] text-black rounded-full shadow-sm">
              SAM
            </span>
          </div>
        )}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 right-4 sm:right-6 w-[calc(100vw-2rem)] sm:w-[500px] h-[640px] max-h-[85vh] bg-[#060108]/96 backdrop-blur-2xl border-2 border-[#360a25] rounded-2xl shadow-[0_16px_50px_rgba(0,0,0,0.95),0_0_30px_rgba(255,23,68,0.25)] z-50 flex flex-col overflow-hidden text-white"
          >
            {/* 1. Header (Fixed Height) */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-[#360a25] bg-[#030004]/90 backdrop-blur-md flex-shrink-0">
              <div className="flex items-center gap-2.5">
                <div className="relative">
                  <motion.div
                    className="w-8 h-8 bg-gradient-to-br from-[#ff1744] via-[#be123c] to-[#880815] rounded-xl flex items-center justify-center shadow-[0_0_14px_rgba(255,23,68,0.5)] border border-white/20"
                  ...
                  <span className={`absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full border border-[#060108] ${
                    isSpeaking ? 'bg-[#00f2fe]' : voiceState === 'awake' || voiceState === 'command-listening' ? 'bg-[#ff1744]' : 'bg-[#00ff88]'
                  } animate-pulse`} />
                </div>

                <div>
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <h3 className="text-sm font-black text-white font-mono tracking-wider">SAM</h3>
                    <span className="text-[9px] font-bold bg-[#ff1744]/15 text-[#ff1744] border border-[#ff1744]/30 px-1.5 py-0.2 rounded-full flex items-center gap-0.5">
                      <Sparkles className="w-2.5 h-2.5" /> GEMINI AI
                    </span>
                    {voiceEnabled && (
                      <span className="text-[9px] font-bold bg-[#00ff88]/15 text-[#00ff88] border border-[#00ff88]/30 px-1.5 py-0.2 rounded-full flex items-center gap-0.5">
                  ...
                  <p className="text-[10px] text-zinc-400 font-medium">
                    {isSpeaking ? (
                      <span className="text-[#00f2fe] font-bold">🔊 Speaking Response...</span>
                    ) : voiceState === 'command-listening' ? (
                      <span className="text-[#ff1744] font-bold">🎙️ Listening to your voice...</span>
                    ) : voiceState === 'processing' ? (
                      <span className="text-[#fbbf24] font-bold">⚡ Querying Gemini AI...</span>
                    ) : (
              ...
              <div className="flex items-center gap-1">
                {ttsSupported && (
                  <button
                    ...
                    className={`p-1.5 rounded-lg transition-all ${ttsEnabled ? 'bg-[#ff1744]/20 text-[#ff1744] border border-[#ff1744]/40 hover:bg-[#ff1744]/30' : 'hover:bg-[#0e0212] text-zinc-500 border border-transparent'}`}
                  >
                ...
                {voiceSupported && (
                  <button
                    ...
                    className={`p-1.5 rounded-lg transition-all ${voiceEnabled ? 'bg-[#00ff88]/20 text-[#00ff88] border border-[#00ff88]/40 hover:bg-[#00ff88]/30' : 'hover:bg-[#0e0212] text-zinc-500 border border-transparent'}`}
                  >
                ...
                <button 
                  onClick={handleClose} 
                  className="p-1.5 rounded-lg hover:bg-[#0e0212] transition-colors text-zinc-400 hover:text-white border border-transparent hover:border-[#360a25]"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
            ...
            <AnimatePresence>
              {isSpeaking && (
                <motion.div
                  ...
                  className="flex-shrink-0 overflow-hidden bg-[#ff1744]/10 border-b border-[#ff1744]/25"
                >
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex gap-0.5 items-end h-3">
                        {[1, 2, 3, 4, 5].map(n => (
                          <motion.div
                            key={n}
                            className="w-0.5 bg-[#ff1744] rounded-full"
                            animate={{ height: [`${3 + n % 3 * 3}px`, `${8 + n % 4 * 3}px`, `${3 + n % 2 * 3}px`] }}
                            transition={{ duration: 0.45 + n * 0.08, repeat: Infinity, delay: n * 0.06 }}
                          />
                        ))}
                      </div>
                      <span className="text-[10px] font-bold text-[#ff1744]">SAM is speaking response...</span>
                    </div>
                    <button
                      onClick={stopSpeaking}
                      className="text-[9px] text-zinc-300 hover:text-white bg-[#ff1744]/20 hover:bg-[#ff1744]/40 px-2 py-0.5 rounded border border-[#ff1744]/40 font-bold transition-all"
                    >
                      Mute Audio
                    </button>
                  </div>
            ...
            <div className="px-3.5 py-2 flex flex-wrap gap-1 flex-shrink-0 border-b border-[#360a25]/60 bg-[#030004]/50">
              <span className="text-[8.5px] text-zinc-400 uppercase tracking-wider font-bold w-full flex items-center gap-1 mb-0.5">
                <Navigation className="w-2.5 h-2.5 text-[#ff1744]" /> Quick Commands
              </span>
              {QUICK_COMMANDS.map(q => (
                <button
                  key={q.cmd}
                  onClick={() => processCommand(q.cmd, false)}
                  className="text-[9.5px] bg-[#060108] border border-[#360a25] text-zinc-300 hover:text-white hover:border-[#ff1744] hover:bg-[#ff1744]/10 px-2 py-0.5 rounded-lg transition-all font-medium"
                >
                  {q.label}
                </button>
              ))}
            ...
            <div className="flex-1 min-h-0 overflow-y-auto px-4 py-3 space-y-3">
              {messages.map(msg => (
                <div key={msg.id} className={`flex gap-2.5 ${msg.type === 'user' ? 'flex-row-reverse' : ''}`}>
                  {/* Avatar */}
                  {msg.type === 'bot' ? (
                    <div className={`w-7 h-7 bg-gradient-to-br from-[#ff1744] via-[#be123c] to-[#880815] rounded-lg flex items-center justify-center flex-shrink-0 shadow-sm mt-0.5 transition-all border border-white/20 ${speakingMsgId === msg.id ? 'ring-2 ring-[#ff1744]' : ''}`}>
                    ...
                    <div className="w-7 h-7 bg-gradient-to-br from-[#00f2fe] to-[#0284c7] rounded-lg flex items-center justify-center flex-shrink-0 shadow-sm mt-0.5 border border-white/20">
                  ...
                  <div className={`group rounded-xl px-3.5 py-2.5 max-w-[380px] relative ${
                    msg.type === 'user'
                      ? `${msg.isVoice ? 'bg-[#fbbf24]/15 border border-[#fbbf24]/30 text-white' : 'bg-[#ff1744]/15 border border-[#ff1744]/30 text-white'} text-[11.5px] leading-relaxed font-medium`
                      : msg.isNav
                      ? 'bg-[#00ff88]/10 border border-[#00ff88]/30 text-[#00ff88] text-[11.5px] font-bold'
                      : 'bg-[#030004] border border-[#360a25] text-white w-full'
                  }`}>
                    {msg.type === 'user' || msg.isNav ? (
                      <span className="text-[11.5px] flex items-center gap-1.5">
                        {msg.isVoice && <Mic className="w-3 h-3 text-[#fbbf24] flex-shrink-0" />}
                          ...
                          <button
                            ...
                            className={`mt-2 flex items-center gap-1 text-[9.5px] transition-all rounded px-2 py-0.5 border font-mono font-bold ${
                              speakingMsgId === msg.id
                                ...
                                : 'text-zinc-500 hover:text-white border-transparent hover:border-[#360a25] hover:bg-[#060108]'
                            }`}
                          >
                            {speakingMsgId === msg.id ? (
                              <><VolumeX className="w-2.5 h-2.5" /> Stop Voice</>
                            ) : (
                              <><PlayCircle className="w-2.5 h-2.5 text-[#00f2fe]" /> Speak Out Loud</>
                            )}
                          </button>
                        )}
                      </>
              ...
              {isLoading && (
                <div className="flex gap-2.5">
                  <div className="w-7 h-7 bg-gradient-to-br from-[#ff1744] to-[#880815] rounded-lg flex items-center justify-center flex-shrink-0 shadow-sm border border-white/20">
                    <Bot className="w-3.5 h-3.5 text-white" />
                  </div>
                  <div className="bg-[#030004] border border-[#360a25] rounded-xl px-4 py-3">
                    <div className="flex gap-1.5 items-center">
                      <div className="w-1.5 h-1.5 bg-[#ff1744] rounded-full animate-bounce" />
                      <div className="w-1.5 h-1.5 bg-[#ff1744]/70 rounded-full animate-bounce" style={{ animationDelay: '0.15s' }} />
                      <div className="w-1.5 h-1.5 bg-[#ff1744]/40 rounded-full animate-bounce" style={{ animationDelay: '0.3s' }} />
                    </div>
                  </div>
                </div>
              )}
            ...
            <AnimatePresence>
              {(voiceState === 'command-listening' || interimText) && (
                <motion.div
                  ...
                  animate={{ height: 'auto', opacity: 1 }}
                  exit={{ height: 0, opacity: 0 }}
                  className="flex-shrink-0 bg-[#0e0212] border-t border-[#360a25] px-3.5 py-1.5 flex items-center justify-between gap-2"
                >
                  ...
                  <button
                    ...
                    className="text-[9px] bg-[#00f2fe]/20 text-[#00f2fe] hover:bg-[#00f2fe]/30 border border-[#00f2fe]/40 px-2 py-0.5 rounded font-bold transition-all flex-shrink-0"
                  >
                    Send Now
                  </button>
                </motion.div>
            ...
            <div className="px-3.5 py-3 border-t border-[#360a25] flex-shrink-0 bg-[#030004]/95 backdrop-blur-md">
              <div className="flex gap-2 items-center">
                {voiceSupported && (
                  <button
                    ...
                    className={`w-9 h-9 flex items-center justify-center rounded-xl transition-all flex-shrink-0 ${
                      voiceState === 'command-listening'
                        ? 'bg-[#ff1744]/20 border border-[#ff1744]/50 text-[#ff1744] shadow-[0_0_12px_rgba(255,23,68,0.4)]'
                        : 'bg-[#060108] border border-[#360a25] hover:border-[#ff1744] text-zinc-400 hover:text-white'
                    }`}
                  >
                    <Mic className={`w-4 h-4 ${voiceState === 'command-listening' ? 'animate-pulse text-[#ff1744]' : ''}`} />
                  </button>
                )}
                ...
                <input
                  ...
                  className="flex-1 bg-[#060108] border border-[#360a25] rounded-xl px-3.5 py-2 text-xs text-white placeholder-zinc-500 focus:outline-none focus:border-[#ff1744] focus:ring-1 focus:ring-[#ff1744]/40 transition-all font-medium"
                />

                <button
                  onClick={() => processCommand(message, false)}
                  disabled={isLoading || !message.trim()}
                  className="w-9 h-9 flex items-center justify-center bg-gradient-to-br from-[#ff1744] to-[#880815] hover:brightness-110 disabled:opacity-40 rounded-xl transition-all shadow-[0_0_12px_rgba(255,23,68,0.4)] border border-white/20 flex-shrink-0"
                >
                  <Send className="w-4 h-4 text-white" />
                </button>
              </div>

              <div className="flex items-center justify-between mt-1.5 font-mono text-[8.5px] text-zinc-500">
