    ...
    <div className="mt-2">
      <div className="flex items-center justify-between mb-2.5">
        <div className="flex items-center gap-2">
          <AlertTriangle className="w-4 h-4 severity-critical" />
          <h2 className="text-xs font-black text-white uppercase tracking-widest text-glow">RECENT CRITICAL ALERTS</h2>
          <span className="text-[9.5px] font-mono font-bold text-red-400 bg-red-500/10 border border-red-500/30 px-2 py-0.5 rounded-full">LIVE TELEMETRY</span>
        </div>
        <button
          onClick={handleDownloadReport}
          disabled={downloading}
          className="flex items-center gap-1.5 px-2.5 py-1 bg-gradient-to-r from-[#ff1744] via-[#be123c] to-[#880815] hover:brightness-110 text-white font-black text-[10.5px] uppercase tracking-wider rounded-lg shadow-[0_0_10px_rgba(255,23,68,0.4)] border border-white/20 transition-all disabled:opacity-50"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5">
        {alerts.slice(0, 3).map((alert, index) => (
          <Link
            ...
            href="/alerts"
            prefetch={true}
            className={`bg-card border-2 border-[#360a25] rounded-xl p-3 hover:border-[#ff1744] transition-all duration-200 cursor-pointer flex flex-col justify-between h-[145px] ${getSeverityGlow(alert.severity)}`}
          >
            <div>
              <div className="flex items-start justify-between mb-1.5">
                <div className="flex items-center gap-1.5 font-mono">
                  <AlertTriangle className={`w-3.5 h-3.5 ${getSeverityColor(alert.severity).split(' ')[0]}`} />
                  <span className={`text-[8.5px] font-black uppercase tracking-wider ${getSeverityColor(alert.severity)} px-1.5 py-0.5 rounded border border-[#360a25]`}>
                    {alert.severity || 'CRITICAL'}
                  </span>
                </div>
                <span className="text-[9.5px] text-zinc-400 font-mono font-bold">{formatAlertTime(alert.time)}</span>
              </div>
              <h3 className="text-[11px] font-black text-white mb-1 line-clamp-1 leading-tight tracking-wide group-hover:text-[#ff1744] transition-colors">{alert.title}</h3>
              <div className="flex items-center gap-1.5 mb-1">
                <span className="text-[8.5px] font-bold text-zinc-300 bg-black/60 border border-[#360a25] px-1.5 py-0.5 rounded">{alert.source || 'Unknown'}</span>
                {alert.affectedSystems && (
                  <span className="text-[8.5px] font-bold text-red-300 bg-red-950/40 border border-red-500/30 px-1.5 py-0.5 rounded">{alert.affectedSystems} Systems</span>
                )}
              </div>
            </div>
            <p className="text-[9.5px] text-zinc-400 font-medium line-clamp-2 mt-auto leading-relaxed">{alert.description}</p>
            {alert.status && (
              <div className="mt-1 pt-1 border-t border-[#360a25]">
                <span className={`text-[8.5px] font-black uppercase tracking-wider ${
                  alert.status === 'Active' ? 'severity-critical text-glow-critical' :
        ...
        <Link 
          href="/alerts"
          prefetch={true}
          className="bg-card border-2 border-[#360a25] rounded-xl p-3 flex flex-col items-center justify-center gap-1.5 hover:border-[#ff1744] transition-all duration-200 hover:shadow-[0_0_18px_rgba(255,23,68,0.35)] group h-[145px]"
        >
          <span className="text-xs font-black uppercase tracking-widest text-[#ff1744] group-hover:text-white transition-colors">VIEW ALL ALERTS</span>
          <ArrowRight className="w-4 h-4 text-[#ff1744] group-hover:translate-x-1 transition-transform" />
        </Link>
      </div>
    </div>
  )
